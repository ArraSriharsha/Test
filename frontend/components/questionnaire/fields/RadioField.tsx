...
/** Rounded shell used for radio sets; order badge sits outside so it lines up with other questions. */
const radioSetShellBase =
  "box-border flex min-w-0 flex-1 flex-col rounded-[36px] border border-sky-500/10 bg-sky-50/70 p-3";
...
export function RadioField({ question, value, onChange }: Props) {
  ...
  if (isBinary) {
    return (
      <div className="flex w-full items-start gap-2 sm:items-center">
        ...
        <div className={`${radioSetShellBase} gap-3`}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <span className="min-w-0 flex-1 text-[13px] font-bold leading-snug text-[#0C1A3A]">
              {question.label}
            </span>
            <div className="box-border flex h-[38px] w-full max-w-[140px] shrink-0 rounded-full border border-slate-200/80 bg-white p-1 sm:w-[140px]">
              {opts.map((opt) => (
                <button
                  key={opt}
                  type="button"
  ...
  return (
    <div className="flex w-full items-start gap-2">
      <OrderBadge order={question.order} />
      <div className={`${radioSetShellBase} gap-2`}>
        <span className="text-[13px] font-bold leading-snug text-[#0C1A3A]">{question.label}</span>
        {question.description ? (
          <p className="mb-1 w-full text-[11px] font-medium leading-relaxed text-slate-500">{question.description}</p>
        ) : null}
