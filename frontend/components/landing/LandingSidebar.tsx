...
export function LandingSidebar() {
  ...
  return (
    <aside
      className="fixed bottom-[var(--landing-sidebar-gap)] left-[var(--landing-outset)] top-[calc(var(--landing-header-h)+var(--landing-sidebar-gap))] z-40 flex w-[var(--landing-sidebar-w)] flex-col gap-1 overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white/95 p-1 shadow-[0_12px_40px_-20px_rgba(15,23,42,0.14)] backdrop-blur-sm supports-[backdrop-filter]:bg-white/90"
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              title={item.label}
              className={`flex min-h-0 min-w-0 flex-1 basis-0 flex-col justify-between rounded-xl px-2 py-2 outline-none ring-dent-sky transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white ${
